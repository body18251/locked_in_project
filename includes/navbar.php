...
try {
    ...
    // استعلام لاختيار الصفوف حيث يكون الـ status يساوي 0
    $query = "SELECT `id`, `phone`, `message`, `status`, `time` FROM `customer_error` WHERE `status` = 0";
    $statement = $pdo->prepare($query);
    $statement->execute();
    
    // التحقق مما إذا كانت هناك صفوف تطابق الشرط
    if ($statement->rowCount() > 0) {
        // إضافة الكود HTML إذا كان هناك صفوف
        echo '<div class="pulse-css"></div>';
    }
...
$query = "SELECT `id`, `phone`, `message`, `time` FROM `customer_error` WHERE `status` = 0";
...
try {
    ...
    if ($statement->rowCount() > 0) {
        ...
        while ($row = $statement->fetch(PDO::FETCH_ASSOC)) {
            ...
            $message = $row['message'];
            $time = $row['time'];
            // استخراج أول 50 حرف من الرسالة
            $shortMessage = substr($message, 0, 50);
            echo "<li class='media dropdown-item align-items-center gap-3 open-modal' data-id='$id' data-phone='$phone' data-message='$message' data-time='$time'>";
            echo "<span class='success'><i class='ti-user'></i></span>";
            echo "<div class='media-body'>";
            echo "<a href='#'>$phone: $shortMessage</a>";
            echo "</div>";
            echo "<span class='notify-time'>$time</span>";
            echo "</li>";
        }
    } else {
        // في حالة عدم وجود بيانات
        echo "<li class='media dropdown-item align-items-center gap-3'>";
        echo "<div class='media-body'>";
        echo "<p>no massage found</p>";
        echo "</div>";
        echo "</li>";
    }
} catch (PDOException $e) {
    ...
    openModalLinks.forEach(function(link) {
        link.addEventListener('click', function(event) {
            ...
            swal({
                ...
                if (value) {
                    // إذا تم النقر على زر Done
                    // يجب إرسال طلب لتحديث الحالة إلى 1
                    // يمكنك استخدام AJAX
                    const xhr = new XMLHttpRequest();
                    xhr.onreadystatechange = function() {
                        if (xhr.readyState === 4 && xhr.status === 200) {
                            // يمكنك إدراج أي إجراءات تحتاجها بعد تحديث الحالة هنا
                            // مثلاً: إعادة تحميل الصفحة بعد التحديث
                            location.reload();
                        }
                    };
